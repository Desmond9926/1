...
def choose_s2(s1: Point, theta_deg: float, now: Point) -> Point:
    plus, minus = s2_candidates(s1, theta_deg)
    dplus, dminus = dist(plus, now), dist(minus, now)
    if abs(dplus - dminus) > 1.0:
        pick = plus if dplus < dminus else minus
    else:
        pick = plus if hypot(plus) <= hypot(minus) else minus
    if s2_receive_ok(s1, theta_deg, pick):
        return pick
    other = minus if pick is plus else plus
    if s2_receive_ok(s1, theta_deg, other):
        return other
    return pick


def closer_s2_offsets(s1: Point, theta_deg: float, now: Point) -> List[Point]:
    """Fallback second/third stations, still with a lateral offset."""
    u = unit_from_deg(theta_deg)
    n = left_normal(u)
    specs = [(750.0, 600.0), (750.0, -600.0), (400.0, 350.0), (400.0, -350.0),
             (200.0, 250.0), (200.0, -250.0)]
    pts: List[Point] = []
    for a, b in specs:
        p = add(s1, add(scale(u, a), scale(n, b)))
        pts.append(p)
